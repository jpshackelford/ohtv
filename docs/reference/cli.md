...
| Command | Purpose | Detailed docs |
|---|---|---|
| `ohtv list` | List conversations with filters (date, PR, repo, action, label). Opt-in `--with-engagement` adds engagement columns (`Engaged` / `Periods` / `Eng%`) to the table and matching fields to JSON / CSV. Opt-in `--with-outcomes` adds an outcomes column showing PRs/issues with state indicators (✓ for merged/closed, → for open). `--enriched` is shorthand for `--with-engagement --with-outcomes`. Engagement-aware row filtering via `--engaged` / `--no-engaged` / `--min-engaged DURATION` / `--min-engagement-ratio PCT` — see [exploration § Engagement filters](../guides/exploration.md#engagement-filters--engaged---no-engaged---min-engaged---min-engagement-ratio). Engagement requires the [`engagement` stage](../guides/indexing.md#engagement-stage); outcomes require the [`refs`](../guides/indexing.md#refs-stage) and [`actions`](../guides/indexing.md#actions-stage) stages plus [`conversation_contributions`](../guides/indexing.md#contributions-stage). | [guides/exploration.md § list](../guides/exploration.md) |
