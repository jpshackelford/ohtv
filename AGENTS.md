...
27. **Manifest as canonical metadata source + `sync.lock` writer mutex (Issues #86, #87, #109)**: The sync manifest (`~/.ohtv/sync_manifest.json`) is the authoritative cache for cloud-side editable metadata. #86 covered `title` and `labels`; #87 extends this to `selected_repository`, `selected_branch`, and `created_at`. The scanner (`db/scanner.py:extract_metadata`) prefers a non-empty manifest title over `base_state.json`, and for cloud convs whose manifest entry carries all three #87 keys, **skips `base_state.json` entirely** — a cold `db scan --force` over a manifest-populated dataset opens `base_state.json` only for local CLI convs.
